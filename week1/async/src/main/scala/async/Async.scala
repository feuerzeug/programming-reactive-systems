...
object Async extends AsyncInterface {
  ...
  /**
    ...
    */
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = eventuallyX.flatMap(x => Future{x % 2 == 0})
  ...
  /**
    ...
    */
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] = eventuallyX.recover{ case _: Throwable => -1}
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1().flatMap(x => makeAsyncComputation2().map((x,_)))
  ...
  /**
    ...
    */
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = if (maxAttempts==1) makeAsyncComputation() else
    makeAsyncComputation().recoverWith{case e: Exception => insist(makeAsyncComputation,maxAttempts-1)}
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = {
    class Fba extends FutureBasedApi {
